...
const answer = multiply(4, 3, callback)
...
const firstName = (arr) => arr

const first = (arr, callBack2) => {
  return callBack2(arr[0])
}
...
const lastName = (arr) => arr

const last = (arr, callback3) => {
  return callback3(arr[6])
}
...
const contains = (arr, personName, callback4) => {
    if (arr.includes(personName)) {
      callback4(true)
    } else {
      callback4(false)
    }
...
const cb1 = (item, index) => {
  console.log(`The item at index ${item} is ${index}`)
}

const each = (arr, cb) => {
  arr.forEach((nombre, index) => cb(nombre, index))
}
